...
impl Server {
    ...
    fn get_property_as_string(
        id: ServerId,
        property: VirtualServerProperties,
    ) -> Result<String, Error> {
        unsafe {
    ...
    fn remove_channel(&mut self, channel_id: ChannelId) -> Option<Channel> {
        self.channels
            .as_mut()
            .ok()
            .and_then(|mut cs| cs.remove(&channel_id))
    }
    ...
    pub fn get_mut_channel(&mut self, channel_id: ChannelId) -> Option<&mut Channel> {
        self.channels
            .as_mut()
            .ok()
            .and_then(|cs| cs.get_mut(&channel_id))
    }
    ...
    pub fn play_wave_file(&self, path: &str) -> Result<(), Error> {
        unsafe {
            ...
            let res: Error = transmute((TS3_FUNCTIONS
                .as_ref()
                .expect("Functions should be loaded")
                .play_wave_file)(self.id.0, path.as_ptr()));
            match res {
                Error::Ok => Ok(()),
                _ => Err(res),
            }
    ...
    pub fn play_wave_file_handle(
        ...
        unsafe {
            ...
            let res: Error = transmute((TS3_FUNCTIONS
                .as_ref()
                .expect("Functions should be loaded")
                .play_wave_file_handle)(
                self.id.0, path.as_ptr(), play_loop as i32, wave_handle
            ));
            match res {
                Error::Ok => Ok(()),
                _ => Err(res),
    ...
    pub fn pause_wave_file_handle(
        ...
        unsafe {
            let res: Error = transmute((TS3_FUNCTIONS
                ...
                self.id.0, wave_handle, pause as i32
            ));
            match res {
                Error::Ok => Ok(()),
                _ => Err(res),
    ...
    pub fn close_wave_file_handle(
        ...
        unsafe {
            let res: Error = transmute((TS3_FUNCTIONS
                .as_ref()
                .expect("Functions should be loaded")
                .close_wave_file_handle)(
                self.id.0, wave_handle
            ));
            match res {
                Error::Ok => Ok(()),
                _ => Err(res),
    ...
    pub fn print_message<S: AsRef<str>>(&self, message: S, target: MessageTarget) {
        unsafe {
            ...
            (TS3_FUNCTIONS
                .as_ref()
                .expect("Functions should be loaded")
                .print_message)(self.id.0, text.as_ptr(), target);
        }
    }
}
...
impl Connection {
    ...
    #[doc(hidden)]
    pub fn get_client_property_as_string(
        server_id: ServerId,
        id: ConnectionId,
    ...
    fn load(&mut self) -> Result<(), Error> {
        ...
        let res: Error = unsafe {
            transmute((TS3_FUNCTIONS
                .as_ref()
                .expect("Functions should be loaded")
                .get_server_connection_handler_list)(
                &mut result
            ))
        };
        match res {
        ...
        unsafe {
            TsApi::get_path(|p, l| {
                (TS3_FUNCTIONS
                    ...
                    .expect("Functions should be loaded")
                    .get_plugin_path)(
                    p, l, to_cstring!(self.plugin_id.as_str()).as_ptr()
                )
            })
        }
    }
