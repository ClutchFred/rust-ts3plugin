...
pub unsafe extern "C" fn ts3plugin_onServerEditedEvent(
    ...
    (::TS3_FUNCTIONS.as_ref().unwrap().request_connection_info)(
        server_id,
        invoker_id,
        0 as *const c_char,
    );
...
pub unsafe extern "C" fn ts3plugin_onClientMoveEvent(
    ...
    if old_channel_id == ::ChannelId(0) {
        ...
        // Update the channel
        {
            if let Some(connection) = api
                .get_mut_server(server_id)
...
pub unsafe extern "C" fn ts3plugin_onClientMoveMovedEvent(
    ...
    if old_channel_id == ::ChannelId(0) {
        ...
        if visibility == Visibility::Leave {
            api.get_mut_server(server_id)
                .map(|s| s.remove_connection(connection_id));
        }
    }
}
...
pub unsafe extern "C" fn ts3plugin_onChannelDescriptionUpdateEvent(
    ...
    if let Err(error) = if let Some(channel) = api
        .get_mut_server(server_id)
        .unwrap()
...
pub unsafe extern "C" fn ts3plugin_onDelChannelEvent(
    ...
    if api
        .get_mut_server(server_id)
        .and_then(|s| s.remove_channel(channel_id))
        .is_none()
    {
...
pub unsafe extern "C" fn ts3plugin_onUpdateChannelEditedEvent(
    ...
    if let Err(error) = {
        ...
        plugin.channel_edited(api, server_id, channel_id, old_channel, invoker);
    }
}
...
pub unsafe extern "C" fn ts3plugin_onChannelMoveEvent(
    ...
    if let Some(channel) = api
        .get_mut_server(server_id)
        .and_then(|s| s.get_mut_channel(channel_id))
    {
...
pub unsafe extern "C" fn ts3plugin_onTextMessageEvent(
    ...
    let message_receiver = match target_mode {
        ::TextMessageTargetMode::Client => ::MessageReceiver::Connection(receiver_id),
...
pub unsafe extern "C" fn ts3plugin_onAvatarUpdated(
    ...
    let server_id = ::ServerId(server_id);
    let connection_id = ::ConnectionId(connection_id);
    let path = if avatar_path.is_null() {
        None
    } else {
        Some(to_string!(avatar_path))
    };
    let mut data = DATA.lock().unwrap();
    let mut data = data.0.as_mut().unwrap();
    let mut api = &mut data.0;
...
pub unsafe extern "C" fn ts3plugin_onServerGroupClientAddedEvent(
    ...
    let mut plugin = &mut data.1;
    api.try_update_invoker(server_id, &invoker);
    plugin.connection_server_group_added(api, server_id, connection, server_group_id, invoker);
}
...
pub unsafe extern "C" fn ts3plugin_onServerGroupClientDeletedEvent(
    ...
    let mut plugin = &mut data.1;
    api.try_update_invoker(server_id, &invoker);
    plugin.connection_server_group_removed(api, server_id, connection, server_group_id, invoker);
}
...
pub unsafe extern "C" fn ts3plugin_onServerPermissionErrorEvent(
    ...
    let mut api = &mut data.0;
    let mut plugin = &mut data.1;
    if plugin.permission_error(api, server_id, permission_id, error, message, return_code) {
        1
    } else {
        0
    }
...
pub unsafe extern "C" fn ts3plugin_onEditPostProcessVoiceDataEvent(
    ...
    sample_count: c_int,
    channels: c_int,
    channel_speaker_array: *const c_uint,
    channel_fill_mask: *mut c_uint,
) {
    let server_id = ::ServerId(server_id);
    let connection_id = ::ConnectionId(connection_id);
    let mut samples = slice::from_raw_parts_mut(samples, (sample_count * channels) as usize);
...
pub unsafe extern "C" fn ts3plugin_onEditCapturedVoiceDataEvent(
    ...
    *edited |= plugin.captured_voice_data(api, server_id, samples, channels, &mut send) as c_int;
